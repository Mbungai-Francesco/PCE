...
import { deleteMissionDrone, getMissionDroneById } from "@/api/MissionDroneApi";
...
export const FormPage = () => {
  ...
  const navigate = useNavigate();
	...
	const { setMissionData, setGeneraleData, setTechData, setAdminData } =
		useData();
    ...
    if(hasJwt()){
      deleteMissionDrone(id || "").then(() => {
        console.log("Mission Drone deleted successfully");
        navigate("/")
      }).catch((error) => {
        console.error("Error deleting Mission Drone:", error);
        // navigate("/form")
      });
    }
    else navigate("/")
	...
	useEffect(() => {
		if (hasJwt()) {
			getMissionDroneById(getJwt() || "").then((res) => {
				...
				console.log(
					"Date vol:",
					new Date(res.dateDebutVol).toISOString().split("T")[0]
				);
				setMissionData(res);
	...
	return (
		<div
			className={
				"bg-linear-to-br from-blue-400 to-purple-700 min-h-screen w-full pt-10"
			}
		>
			<div className={cn("w-1/2 h-fit min-h-10 mx-auto")}>
				<div
					className={cn(
						"w-full flex justify-between items-center py-7 px-8 bg-sky-900 rounded-t-xl"
					)}
				>
					<Link to="/">
						<p
							className={cn(
								"text-sky-900 bg-white w-fit h-fit py-2 px-5 rounded-md font-bold tracking-wider text-2xl"
							)}
						>
							CEREMA
						</p>
					...
					<div className={cn("w-1/2 flex flex-col gap-2")}>
						...
						<div className={cn("w-full h-2 bg-blue-100/80 rounded-full ")}>
							<div
								ref={progressRef}
								className={cn(
									"bg-linear-to-br from-green-600 to-green-300 h-2 rounded-full"
								)}
							></div>
						</div>
					</div>
				...
				<div
					className={cn(
						"w-full min-h-10 bg-white rounded-b-xl py-7 px-8 text-black"
					)}
				>
